...
/// <summary>
/// Visitor for validating Rust code during unparsing.
/// Checks for Rust-specific issues and ensures the graph can be unparsed to valid Rust.
/// </summary>
public class RustValidationVisitor : SymbolicAnalysisVisitorBase, ISymbolicAnalysisVisitor
{
    ...
    private void VisitPackedNode(CognitiveGraph.PackedNode packedNode)
    {
        ...
        else if (nodeType == "function_declaration" && nodeType == "main")
        {
    ...
    private void ValidateAmbiguity(CognitiveGraph.SymbolNode node, CognitiveGraph.PackedNodeOffsetCollection packedNodes)
    {
        ...
        {
            AddWarning(node, "RV005", $"High ambiguity: {packedNodes.Count} PackedNodes", ValidationSeverity.Warning);
        }
    }
