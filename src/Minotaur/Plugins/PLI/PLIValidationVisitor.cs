...
/// <summary>
/// Visitor for validating PL/I code during unparsing.
/// Checks for PL/I-specific issues and ensures the graph can be unparsed to valid PL/I.
/// </summary>
public class PLIValidationVisitor : SymbolicAnalysisVisitorBase, ISymbolicAnalysisVisitor
{
    ...
    private bool _hasProgramDeclaration = false;
    private bool _inProcedure = false;

    /// <summary>
    /// Initializes a new instance.
    /// </summary>
    public PLIValidationVisitor()
    {
    }
    ...
    public override void Visit(CognitiveGraphNode node)
    {
        ...
        {
            AddError(node, "PV002", "Maximum nesting depth exceeded", ValidationSeverity.Error);
            return;
        }
    ...
    private void VisitPackedNode(CognitiveGraph.PackedNode packedNode)
    {
        ...
        // Track program declaration
        if (nodeType == "program_declaration")
        {
            _hasProgramDeclaration = true;
        }
        ...
        else if (nodeType == "program_declaration" || nodeType == "end_program")
        {
            _inProcedure = false;
        }
    ...
    private void ValidateLeafNode(CognitiveGraph.SymbolNode node)
    {
        ...
        {
            AddWarning(node, "PV003", "Empty node content", ValidationSeverity.Warning);
        }
        
        if (node.NodeType == 0)
        {
            AddError(node, "PV004", "Invalid node type (0)", ValidationSeverity.Error);
        }
    }
    ...
    {
        // PL/I can have some ambiguity
        if (packedNodes.Count > 4)
        {
            AddWarning(node, "PV005", $"High ambiguity: {packedNodes.Count} PackedNodes", ValidationSeverity.Warning);
        }
    }
    ...
    private void ValidatePackedNode(CognitiveGraph.PackedNode packedNode)
    {
        ...
        {
            AddError(packedNode, "PV006", "Invalid RuleId (0)", ValidationSeverity.Error);
        }
        
        var childNodes = packedNode.GetChildNodes();
        if (childNodes.Count == 0)
        {
            AddWarning(packedNode, "PV007", "PackedNode with no children", ValidationSeverity.Info);
        }
    }
